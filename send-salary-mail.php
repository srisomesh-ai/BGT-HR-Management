<?php
// ═══════════════════════════════════════════════════════
//  BharatGPS HR — Salary Credited Email Notifier
//  Sends to employee when admin marks salary as paid
// ═══════════════════════════════════════════════════════
...
$netSalary = htmlspecialchars($data['netSalary'] ?? '');
$paidAt    = htmlspecialchars($data['paidAt']    ?? '');
$utr       = htmlspecialchars($data['utr']       ?? '');

if (!$empEmail) { echo json_encode(['ok'=>false,'error'=>'No employee email']); exit; }

$recipients = [$empEmail];
$subject    = "Salary Credited — {$month} {$year} | BharatGPS";
$fmtAmt     = '₹' . number_format((float)$netSalary, 0, '.', ',');
$utrLine    = $utr ? "<div class='row'><div class='lbl'>Reference No.</div><div class='val'>{$utr}</div></div>" : '';
$utrPlain   = $utr ? "Reference No. : {$utr}\n" : '';
...
<style>
  ...
  .hi{font-size:15px;color:#0f1629;margin-bottom:18px;}
  .amt-box{background:#dcfdf0;border:1px solid #a3f0cc;border-radius:10px;padding:18px 20px;text-align:center;margin-bottom:20px;}
  .amt-lbl{font-size:12px;color:#059669;font-weight:700;text-transform:uppercase;letter-spacing:1px;}
  .amt-val{font-size:32px;font-weight:800;color:#0d9e6a;margin-top:4px;}
...
<div class='wrap'>
  <div class='hdr'>
    <div class='hdr-t'>&#128176; Salary Credited</div>
    <div class='hdr-s'>BharatGPS HR Management System</div>
  </div>
  <div class='bd'>
    <div class='hi'>Dear <strong>{$empName}</strong>,</div>
    <div class='amt-box'>
      <div class='amt-lbl'>Net Salary Credited</div>
      <div class='amt-val'>{$fmtAmt}</div>
    </div>
    <div class='row'><div class='lbl'>Employee ID</div><div class='val'>{$empId}</div></div>
    <div class='row'><div class='lbl'>Month</div><div class='val'>{$month} {$year}</div></div>
    <div class='row'><div class='lbl'>Payment Date</div><div class='val'>{$paidAt}</div></div>
    {$utrLine}
    <div class='msg'>
      Your dedication and hard work are truly valued at BharatGPS. Keep up the excellent work,
      stay focused on your goals, and most importantly — <strong>stay safe on the field</strong>.<br><br>
      Wishing you continued success. Keep going strong! &#128640;
    </div>
  </div>
...
$plain = "Dear {$empName},\n\nYour salary for {$month} {$year} has been successfully credited.\n\n"
       . "Employee ID   : {$empId}\n"
       . "Month         : {$month} {$year}\n"
       . "Net Salary    : {$fmtAmt}\n"
       . "Payment Date  : {$paidAt}\n"
       . $utrPlain
       . "\nYour dedication and hard work are truly valued at BharatGPS. Keep up the excellent work, "
       . "stay focused, and stay safe on the field.\n\nWarm regards,\nBharatGPS HR Team";
...
function smtp_send($host, $port, $user, $pass, $from, $fromName, $toList, $subject, $html, $plain) {
    ...
    function smtp_cmd2($sock, $cmd, $expect) {
    ...
    $r = smtp_cmd2($sock, null, '220');                          if(!$r['ok']){fclose($sock);return $r;}
    $r = smtp_cmd2($sock, 'EHLO bharatgps.com', '250');          if(!$r['ok']){fclose($sock);return $r;}
    $r = smtp_cmd2($sock, 'AUTH LOGIN', '334');                   if(!$r['ok']){fclose($sock);return $r;}
    $r = smtp_cmd2($sock, base64_encode($user), '334');           if(!$r['ok']){fclose($sock);return $r;}
    $r = smtp_cmd2($sock, base64_encode($pass), '235');           if(!$r['ok']){fclose($sock);return $r;}
    $r = smtp_cmd2($sock, "MAIL FROM:<{$from}>", '250');          if(!$r['ok']){fclose($sock);return $r;}
    foreach ($toList as $to) { $r = smtp_cmd2($sock, "RCPT TO:<{$to}>", '250'); if(!$r['ok']){fclose($sock);return $r;} }
    $r = smtp_cmd2($sock, 'DATA', '354');                         if(!$r['ok']){fclose($sock);return $r;}
    $boundary = md5(uniqid());
    ...
    $r = smtp_cmd2($sock, $msg, '250'); if(!$r['ok']){fclose($sock);return $r;}
    smtp_cmd2($sock, 'QUIT', null); fclose($sock);
    return ['ok'=>true];
}
